...
function displayMovie(index) {
  ...
  const movieContainerHtml = $(
    '<div class="hidden list-group-item flex-center clearfix movie-container wow"></div>'
  );
  if (index % 2 === 0) {
  ...
  const button = $(
    '<button type="button" onclick="deleteMovieFromFavorites(index)" class="btn btn-danger">Remove</button>'
  );
