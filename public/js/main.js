...
$(document).ready(() => {
  $("html").bind("wheel DOMMouseScroll", () => {
    ...
    for (let i = 0; i < allMoviesNumber; i++) {
      ...
      const elementOffset =
        $(selector).position().top - window.innerHeight - 100;
...
function displayMovie(movie, index) {
  ...
  const movieContainerHtml = $(
    '<div class="hidden movie-container flex-column flex-center"></div>'
  );
  ...
  const infoContainerHtml = $(
    '<div class="hidden flex-custom flex-center"></div>'
  );
  /* Poster */
  ...
  /* Movie Title */
  const movieTitle = $(
    '<p class="first-font-style" style="margin-top: 17px; padding-right: 5px;"></p>'
  ).text(name);
